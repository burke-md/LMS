...
describe("Advanced Nft", function () {
  ...
  describe("Bench mark gas", function () {
    ...
    it("Display gas estimate above", async function () {
      ...
      const gasEstimate = await nft721.connect(user4).whiteListMintBitmap(ticketProofs.user4Proof, 4, {
        value: ethers.utils.parseEther('0.05')
            });
      console.log(`\t\tMerkle proof w/ bitmap: ${gasEstimate.gasLimit.toString()}`);
    });
  });
  describe("Business Logic", function () {
    ...
    it("Should validate special merkle proofs (address hashed w/ ticket", async function () {
      ...
      await nft721.connect(user4).whiteListMintBitmap(ticketProofs.user4Proof, 4, {
        value: ethers.utils.parseEther('0.05')
            });
